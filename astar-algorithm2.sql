...
CREATE OR REPLACE FUNCTION encontrar_camino_astar(
    ...
    max_alternativas INTEGER DEFAULT 1
) RETURNS TABLE (
    camino_id INTEGER,
    camino TEXT,
    costo_total FLOAT,
...
BEGIN
    ...
    -- Depuración: Iniciar la búsqueda
    RAISE NOTICE 'Iniciando búsqueda de camino desde % a %', origen_id, destino_id;
    ...
    WHILE array_length(lista_abierta, 1) > 0 LOOP
        ...
        -- Depuración: Nodo actual que estamos explorando
        RAISE NOTICE 'Explorando nodo: %', nodo_actual;
        ...
        IF nodo_actual = destino_id THEN
            ...
            -- Depuración: Encontramos el camino al destino
            RAISE NOTICE 'Camino encontrado: %', camino;
            ...
            -- Verificar si ya se han encontrado suficientes alternativas
        ...
        -- Depuración: Lista abierta y cerrada
        RAISE NOTICE 'Lista abierta: %, Lista cerrada: %', lista_abierta, lista_cerrada;
        ...
        FOR result IN EXECUTE query_text LOOP
            ...
            -- Depuración: Explorando vecino
            RAISE NOTICE 'Vecino explorado: %', siguiente_nodo;

            -- Verificar si hemos excedido el número de nodos intermedios permitidos
            IF siguiente_nodo = ANY(lista_cerrada) THEN
                CONTINUE;
            END IF;
            IF num_intermedios >= max_alternativas - 1 THEN
                CONTINUE;
            END IF;

            -- Calcular el g_score según el criterio
            ...
            IF NOT (siguiente_nodo = ANY(lista_abierta)) THEN
                ...
                -- Actualizar los caminos, costos, distancias y duraciones
                ...
                num_intermedios := num_intermedios + 1;  -- Incrementar el contador de nodos intermedios

                -- Depuración: Añadir siguiente nodo
                RAISE NOTICE 'Añadiendo nodo a la lista: %, g_score: %, f_score: %',
                             siguiente_nodo, temp_g, temp_f;
            END IF;
...
DECLARE
    lista_abierta TEXT[] := ARRAY[origen_id::TEXT]; -- Nodos por explorar
    lista_cerrada TEXT[] := ARRAY[]::TEXT[]; -- Nodos ya explorados
    nodo_actual TEXT;
    vecino RECORD;
    camino_encontrado BOOLEAN := FALSE;
    ruta_completa TEXT;
    cypher_query TEXT;
BEGIN
    RAISE NOTICE 'Iniciando búsqueda de camino desde % a %', origen_id, destino_id;
    ...
    WHILE array_length(lista_abierta, 1) > 0 LOOP
        -- Extraer el nodo actual desde la lista abierta
        nodo_actual := lista_abierta[1];
        lista_abierta := array_remove(lista_abierta, nodo_actual);
        lista_cerrada := array_append(lista_cerrada, nodo_actual);
        ...
        -- Generar la consulta Cypher que se ejecutará
        cypher_query := format(
            'SELECT * FROM cypher(''el_grefo'', $q$MATCH (n1:Estacion {id: %s})-[r:RUTA]->(n2:Estacion) RETURN n2.id AS id, r.precio_billete AS costo_total, r.distancia AS distancia, r.duracion AS duracion$q$) AS (id agtype, costo_total FLOAT, distancia FLOAT, duracion FLOAT)',
            nodo_actual::INTEGER
        );

        -- Mostrar la consulta Cypher generada
        RAISE NOTICE 'Ejecutando consulta Cypher: %', cypher_query;

        -- Ejecutar la consulta Cypher para obtener los vecinos
        FOR vecino IN
            EXECUTE cypher_query
        LOOP
            RAISE NOTICE 'Vecino explorado: %, Costo: %, Distancia: %, Duración: %', vecino.id, vecino.costo_total, vecino.distancia, vecino.duracion;

            -- Si el vecino es el nodo destino, se encontró el camino
            IF vecino.id::TEXT = destino_id::TEXT THEN
                RAISE NOTICE 'Nodo destino encontrado: %', vecino.id;
                camino_encontrado := TRUE;

                -- Construir y retornar la ruta completa
                ruta_completa := array_to_string(lista_cerrada || ARRAY[vecino.id::TEXT], ',');
                RETURN QUERY
                SELECT
                    1 AS camino_id,
                    ruta_completa AS ruta,
                    vecino.costo_total AS costo,
                    vecino.distancia AS distancia,
                    vecino.duracion AS duracion;

                EXIT;
            END IF;

            -- Si el vecino no está en la lista cerrada, añadirlo a la lista abierta
            IF NOT vecino.id::TEXT = ANY(lista_cerrada) THEN
                IF NOT vecino.id::TEXT = ANY(lista_abierta) THEN
                    RAISE NOTICE 'Añadiendo vecino % a lista abierta', vecino.id;
                    lista_abierta := array_append(lista_abierta, vecino.id::TEXT);
                END IF;
            END IF;
        END LOOP;

        -- Si encontramos un camino, detener la exploración
        IF camino_encontrado THEN
            EXIT;
        END IF;
    END LOOP;

    -- Si no se encuentra ningún camino
    IF NOT camino_encontrado THEN
        RAISE NOTICE 'No se encontró un camino entre % y %', origen_id, destino_id;
    END IF;
END;
$$ LANGUAGE plpgsql;
